...
export class ScheduleAllAppletsExecutionHandler implements ICommandHandler<ScheduleAllAppletsExecutionCommand> {
    ...
    async execute(command: ScheduleAllAppletsExecutionCommand): Promise<void> {
        ...
        await this.messageQueueProvider.bulkPublishAppletExecution(applets.map(applet => applet.id));
