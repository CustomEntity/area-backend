...
export class KafkaService implements OnModuleInit{
    ...
    constructor(
        ...
        assert(this.configService.get<string>('kafka.brokers'), 'Kafka brokers must be defined')
        assert(this.configService.get<string>('kafka.username'), 'Kafka username must be defined')
        assert(this.configService.get<string>('kafka.password'), 'Kafka password must be defined')
        ...
        try {
            ...
            this._consumer = this.kafka.consumer({groupId: this.configService.get<string>('kafka.groupId') || 'my-group'})
        } catch (error) {
            console.error(error)
            throw error
        }
    ...
    async discoverAndSetupConsumers() {
        ...
        for (const instance of instances) {
            ...
            const methodNames = Object.getOwnPropertyNames(prototype).filter(
                item => typeof prototype[item] === 'function' && Reflect.hasMetadata(KAFKA_CONSUMER_METHOD_METADATA, prototype[item]),
            );

            for (const methodName of methodNames) {
                const method = prototype[methodName];
                const topic = Reflect.getMetadata(KAFKA_CONSUMER_METHOD_METADATA, method);
